...
class kinectBodyTracker:
	...
	def get3DSkeleton(self, body):
		skeleton = np.zeros((25, 3))
		for i in range(25): # x, y, x coordinates in meters
			skeleton[i, 0] = body.skeleton.joints[k4a2v2[i]].position.v[0] * 1e-3
			skeleton[i, 1] = body.skeleton.joints[k4a2v2[i]].position.v[1] * 1e-3
			skeleton[i, 2] = body.skeleton.joints[k4a2v2[i]].position.v[2] * 1e-3
		return skeleton
